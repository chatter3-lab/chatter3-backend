...
if(p==='/api/admin/stats'&&req.method==='POST'){
    ...
    if(p==='/api/admin/referrals'&&req.method==='POST'){
      ...
      const recent:any=await env.DB.prepare(`
        SELECT pt.id,pt.user_id,pt.points,pt.created_at,
          u.username as invitee_name,u.nickname as invitee_nickname,u.email as invitee_email,
          ref.username as referrer_name,ref.nickname as referrer_nickname,ref.email as referrer_email
        FROM point_transactions pt
        LEFT JOIN users u ON pt.user_id=u.id
        LEFT JOIN invites i ON i.invitee_id=pt.user_id
        LEFT JOIN users ref ON i.inviter_id=ref.id
